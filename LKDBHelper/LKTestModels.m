...
//        NSString* dbpath = [NSHomeDirectory() stringByAppendingPathComponent:@"sportdata.sqlite"];//@"asd/asd.db"];

//********
        NSString *sqlFile = @"sportdata.sqlite";
        NSArray *cachePath= NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
...
//        NSString *databasePathFromApp = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:sqlFile];
        NSString *databasePathFromApp= [[NSBundle mainBundle] pathForResource:@"sportdata" ofType:@"sqlite"];
...
//********
        db = [[LKDBHelper alloc]initWithDBPath:databasePath];
        //or
...
+(void)initialize
{
    ...
    [self setTableColumnName:@"nameIndex" bindingPropertyName:@"lastName"];
